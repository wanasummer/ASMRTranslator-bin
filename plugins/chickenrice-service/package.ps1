...
$StageDir = Join-Path ([System.IO.Path]::GetTempPath()) ("chickenrice-service-package-" + [guid]::NewGuid())
$Archive = Join-Path $DestinationDir "chickenrice-service-$Version.zip"

New-Item -ItemType Directory -Path $StageDir -Force | Out-Null
New-Item -ItemType Directory -Path $DestinationDir -Force | Out-Null
try {
    $Items = @(
        "src", "pyproject.toml", "plugin-manifest.json", "README.md", ".env.example",
        "install.ps1", "start.ps1", "install.sh", "start.sh", "Dockerfile",
        "docker-entrypoint.sh", "compose.yaml"
    )
